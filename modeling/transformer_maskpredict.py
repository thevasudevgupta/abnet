...
class LossFunc(nn.Module):
    ...
    def compute_length_loss(self, length_logits, length_labels, pad_mask):
        ...
        # print(length_labels.shape)
        # print(num_pads.shape)
        ...
        # print(length_logits)
        # print('------------Labels------------')
        # print(length_labels.shape[1])
        length_logits = length_logits[:,:,:length_labels.shape[1]]#.type(torch.LongTensor)

        # device = 'cuda' if torch.cuda.is_available
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        length_labels = length_labels.type(torch.LongTensor).to(device)
